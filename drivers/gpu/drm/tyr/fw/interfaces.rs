...
mod iface {
    ...
    pub(super) struct FwInterface<const FW_IFACE_SIZE: usize> {
        ...
        vmap: Arc<VMapOwned<BoData, Uninit>>,
        /// Offset within the shared memory buffer where this interface starts.
        offset: usize,
    }

    // SAFETY:
    // - `FwInterface` owns a `VMapOwned`, which keeps the GEM object and mapping alive.
    // - Moving `FwInterface` to another thread does not invalidate the mapped address.
    // - Access to `GlobalInterface` is protected by `Mutex`, so the firmware interface state is not
    //   concurrently mutated through this type.
    unsafe impl<const FW_IFACE_SIZE: usize> Send for FwInterface<FW_IFACE_SIZE> {}
...
impl GlobalInterface {
    ...
    fn init_csg(&mut self, shared_section: &Section<'_>) -> Result {
...
impl CsgInterface {
    ...
    fn init_cs(&mut self, shared_section: &Section<'_>, csg_control_offset: usize) -> Result {
        let enabled = match &mut self.state {
